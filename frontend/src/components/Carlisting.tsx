...
function SearchPage() {
  ...
  function deleteProducts(id) {
    ...
    fetch("http://localhost:4000/car-list/" + id, {
      method: 'delete'
      ...
      </div>
      {
        state.length > 0 ? state.map((item, i) =>
          ...
          <div className="flex" >
           ...
           {JSON.parse(localStorage.getItem('user')).email=='admin@test.com' && JSON.parse(localStorage.getItem('user')).password=='admin1234' && <Link to={"/updateCar/" + item._id} className="mr-[6px]"><FontAwesomeIcon icon={faPenToSquare} color='orange' /></Link>}
        ...
        ) : <div className="h1 ml-[35px]">No Cars found</div>
      }
